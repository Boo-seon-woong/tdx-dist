...
shared region은 host와 guest MN이 같은 `memory_file`을 열어 공유한다고 가정한다.
...
먼저 host가 region을 만들고 RDMA로 노출한다.

```bash
./bin/host --config build/config/host.rdma.conf
```

그 다음 guest MN이 같은 `memory_file`을 attach 해서 request ring을 소비한다.
