...
static int td_cluster_write_value(td_cluster_t *cluster, const char *key, const unsigned char *value, size_t value_len, int update_only, int tombstone, td_latency_profile_t *profile, char *err, size_t err_len) {
...

    {
        ...
        if (td_commit_slot_at(primary, TD_REGION_PRIME, primary_probe.candidate_slot_index, &proposal, current_epoch, &observed, profile != NULL ? &timing : NULL, err, err_len) != 0 ||
            ...
            td_format_error(err, err_len, "primary commit failed for key %s", key);
            return -1;
        }
        if (profile != NULL) {
            profile->primary_write_ns += timing.write_ns;
...
static void td_print_latency_profile(FILE *out, const td_latency_profile_t *profile) {
    ...
    if (profile->transport == TD_TRANSPORT_TCP) {
        ...
        td_print_latency_line(out, "rdma_atomic_post_send", profile->transport_profile.rdma_atomic_post_send_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_atomic_post_send_ns;
        td_print_latency_line(out, "rdma_atomic_poll_cq", profile->transport_profile.rdma_atomic_poll_cq_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_atomic_poll_cq_ns;
        td_print_latency_line(out, "rdma_atomic_backoff", profile->transport_profile.rdma_atomic_backoff_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_atomic_backoff_ns;
        td_print_latency_line(out, "rdma_request_reserve", profile->transport_profile.rdma_request_reserve_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_request_reserve_ns;
        td_print_latency_line(out, "rdma_request_payload_write", profile->transport_profile.rdma_request_payload_write_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_request_payload_write_ns;
        td_print_latency_line(out, "rdma_request_publish", profile->transport_profile.rdma_request_publish_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_request_publish_ns;
        td_print_latency_line(out, "rdma_request_completion_wait", profile->transport_profile.rdma_request_completion_wait_ns, profile->total_ns);
        accounted_ns += profile->transport_profile.rdma_request_completion_wait_ns;
    }
    ...
    if (profile->transport == TD_TRANSPORT_TCP) {
        ...
        td_print_latency_value(out, "rdma_atomic_completion_elapsed", profile->transport_profile.cas_wait_ns);
        td_print_latency_value(out, "rdma_request_reserve_elapsed", profile->transport_profile.rdma_request_reserve_ns);
        td_print_latency_value(out, "rdma_request_publish_elapsed", profile->transport_profile.rdma_request_publish_ns);
        td_print_latency_value(out, "rdma_request_completion_elapsed", profile->transport_profile.rdma_request_completion_wait_ns);
    }
    ...
    td_print_latency_value(out, "workload_snapshot_backup_commit_path", profile->backup_write_ns);
    td_print_latency_value(out, "workload_snapshot_backup_finalize", profile->backup_cas_ns);
    ...
    td_print_latency_value(out, "workload_snapshot_primary_commit_path", profile->primary_write_ns);
    td_print_latency_value(out, "workload_snapshot_primary_finalize", profile->primary_cas_ns);
    ...
    td_print_latency_value(out, "workload_snapshot_repair_commit_path", profile->repair_write_ns);
    td_print_latency_value(out, "workload_snapshot_repair_finalize", profile->repair_cas_ns);
    ...
    td_print_latency_value(out, "workload_cache_refresh_commit_path", profile->refresh_cache_write_ns);
    td_print_latency_value(out, "workload_cache_refresh_finalize", profile->refresh_cache_cas_ns);
    ...
    if (profile->transport == TD_TRANSPORT_RDMA) {
        fprintf(out, "latency.rdma_empty_polls read=%zu write=%zu atomic=%zu ring_full=%zu\n",
            profile->transport_profile.rdma_read_empty_polls,
            profile->transport_profile.rdma_write_empty_polls,
            profile->transport_profile.rdma_atomic_empty_polls,
            profile->transport_profile.rdma_request_ring_full);
        fprintf(out, "latency.rdma_backoffs read=%zu write=%zu atomic=%zu publish_spins=%zu\n",
            profile->transport_profile.rdma_read_backoff_count,
            profile->transport_profile.rdma_write_backoff_count,
            profile->transport_profile.rdma_atomic_backoff_count,
            profile->transport_profile.rdma_request_publish_spins);
    }
