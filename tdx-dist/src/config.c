...
void td_config_init_defaults(td_config_t *cfg) {
    ...
    snprintf(cfg->memory_file, sizeof(cfg->memory_file), "%s", "/tmp/tdx-dist-shm.dat");
}
...
int td_config_load(const char *path, td_config_t *cfg, char *err, size_t err_len) {
    ...
    if (cfg->mode == TD_MODE_HOST && cfg->listen_port <= 0) {
        td_format_error(err, err_len, "host config requires listen_port");
        return -1;
    }
    if ((cfg->mode == TD_MODE_MN || cfg->mode == TD_MODE_HOST) && td_resolve_mn_slots(cfg, err, err_len) != 0) {
        return -1;
    }
    if ((cfg->mode == TD_MODE_CN || cfg->mode == TD_MODE_MN) && cfg->encryption_key_hex[0] == '\0') {
        td_format_error(err, err_len, "encryption_key_hex must be set");
        return -1;
    }
    if ((cfg->mode == TD_MODE_MN || cfg->mode == TD_MODE_HOST) && cfg->memory_file[0] == '\0') {
