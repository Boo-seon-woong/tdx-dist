...
static int td_resolve_slots(td_config_t *cfg, char *err, size_t err_len) {
    size_t header_bytes = sizeof(td_region_header_t);
    size_t ring_bytes = td_request_ring_bytes_for_slots(cfg->request_slots);
    size_t slot_bytes = sizeof(td_slot_t);
    size_t capacity_slots;
    size_t manual_slots;
    size_t remaining_slots;

    if (cfg->request_slots == 0) {
        td_format_error(err, err_len, "request_slots must be larger than zero");
        return -1;
    }
    if (cfg->mn_memory_size <= header_bytes + ring_bytes) {
        td_format_error(err, err_len, "mn_memory_size must be larger than header+ring bytes (%zu)", header_bytes + ring_bytes);
        return -1;
    }

    capacity_slots = (cfg->mn_memory_size - header_bytes - ring_bytes) / slot_bytes;
    if (capacity_slots == 0) {
    ...
    if (manual_slots > capacity_slots) {
        td_format_error(err, err_len,
            ...
            header_bytes + ring_bytes + (manual_slots * slot_bytes),
            cfg->mn_memory_size);
        return -1;
    }
    ...
    if (cfg->prime_slots == 0 || cfg->backup_slots == 0) {
        td_format_error(err, err_len, "resolved slot layout must leave non-zero prime and backup slots");
        return -1;
    }
    return 0;
}
...
void td_config_init_defaults(td_config_t *cfg) {
    ...
    cfg->listen_port = 0;
    cfg->node_id = -1;
    cfg->request_slots = 1024;
    cfg->max_value_size = TD_MAX_VALUE_SIZE;
    snprintf(cfg->listen_host, sizeof(cfg->listen_host), "%s", "0.0.0.0");
    snprintf(cfg->memory_file, sizeof(cfg->memory_file), "%s", "/tmp/tdx-dist-host.dat");
}
...
int td_config_load(const char *path, td_config_t *cfg, char *err, size_t err_len) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        *sep = '\0';
        ...
        } else if (strcmp(key, "request_slots") == 0) {
            if (td_parse_size_literal(value, &cfg->request_slots) != 0) {
                td_format_error(err, err_len, "invalid request_slots at line %zu", line_no);
                fclose(fp);
                return -1;
            }
    ...
    if (cfg->mode != TD_MODE_HOST) {
        td_format_error(err, err_len, "tdx-dist-host only supports mode: host");
        return -1;
    }
    if (cfg->transport != TD_TRANSPORT_RDMA) {
        td_format_error(err, err_len, "tdx-dist-host only supports transport: rdma");
        return -1;
    }
    if (cfg->listen_port <= 0) {
        td_format_error(err, err_len, "host config requires listen_port");
        return -1;
    }
    if (cfg->memory_file[0] == '\0') {
    ...
    return td_resolve_slots(cfg, err, err_len);
}
